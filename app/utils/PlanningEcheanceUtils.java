...
public class PlanningEcheanceUtils {
    ...
    public static Calendrier buildCalendrier(Date date) {
        ...
        DateTime previousMonth = new DateTime(calendrier.actualMonth).minusMonths(
                1).withDayOfMonth(1);
        calendrier.previousMonth = previousMonth.toDate();
        DateTime nextMonth = new DateTime(calendrier.actualMonth).plusMonths(1).withDayOfMonth(
                1);
        calendrier.nextMonth = nextMonth.toDate();

        MutableDateTime currentDate = new MutableDateTime(
                calendrier.actualMonth);
        ...
        while (currentDate.monthOfYear().get() == indexMonth) {
            while (currentDate.weekOfWeekyear().get() == indexWeek
                    && currentDate.monthOfYear().get() == indexMonth) {
                Jour day = new Jour(currentDate);
    ...
    public static boolean isBetween(MutableDateTime date, MutableDateTime from,
            MutableDateTime to) {
    ...
    private static void echeanceMensuelle(Date actualMonth, Echeance echeance) {
        ...
        boolean isInMonth = isBetween(dateEcheance, firstDayOfMonth,
                lastDayOfMonth);
        Logger.debug("isInMonth : [%s]", isInMonth);

        if (isInMonth) {
            models.PlanningEcheance planningEcheance = models.PlanningEcheance.find(
                    "echeance.id=? AND dateEcheance=?", echeance.id,
                    dateEcheance.toDate()).first();
            if (planningEcheance == null) {
                newPlanningEcheance(echeance, dateEcheance.toDate());
                Logger.debug("ajout de l'écheance [%s] au jour [%s]", echeance,
                        echeance.date);
            }
        }
        else {
            if (dateEcheance.isBefore(firstDayOfMonth)) {
                ...
                if (echeance.dateFin == null || dateEcheance.isEqual(dateFin)
                        || (dateEcheance.isBefore(dateFin))) {
                    models.PlanningEcheance planningEcheance = models.PlanningEcheance.find(
                            "echeance.id=? AND dateEcheance=?", echeance.id,
                            dateEcheance.toDate()).first();
                    if (planningEcheance == null) {
                        newPlanningEcheance(echeance, dateEcheance.toDate());
                        Logger.debug("ajout de l'écheance [%s] au jour [%s]",
                                echeance, dateEcheance);
                    }
                }
                else {
                    Logger.debug(
                            "dateEcheance [%s] est apres echeance.dateFin [%s]",
                            dateEcheance, dateFin);
                }
            }
            else {
                Logger.debug(
                        "dateEcheance [%s] est apres firstDayOfMonth [%s]",
                        dateEcheance, firstDayOfMonth);
            }
        }
    }
    ...
    private static void echeanceTriMestrielle(Date actualMonth,
            Echeance echeance) {
    ...
    private static void echeanceAnnuelle(Date actualMonth, Echeance echeance) {
        ...
        boolean isInYear = isBetween(dateEcheance, firstDayOfYear,
                lastDayOfYear);
        ...
        if (isInYear) {
            ...
            if (dateEcheance.isBefore(firstDayOfYear)) {
                ...
                Logger.debug("dateEcheance [%s] est apres firstDayOfYear [%s]",
                        dateEcheance, firstDayOfYear);
            }
        }
    }
    ...
    private static void newPlanningEcheance(Echeance echeance, Date dateEcheance) {
        ...
        planningEcheance.type = echeance.type;
        planningEcheance.dateEcheance = dateEcheance;
        planningEcheance.date = dateEcheance;
        planningEcheance.montant = echeance.montant;
